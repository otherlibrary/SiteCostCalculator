...
if (@$_POST["a_add"] <> "") {
  $worth->CurrentAction = $_POST["a_add"]; // Get form action
  LoadFormValues(); // Load form values
} else { // Not post back
  if ($bCopy) {
    $worth->CurrentAction = "C"; // Copy Record
  } else {
    $worth->CurrentAction = "I"; // Display Blank Record
...
switch ($worth->CurrentAction) {
  ...
  case "C": // Copy an existing record
   if (!LoadRow()) { // Load record based on key
      ...
      Page_Terminate($worth->getReturnUrl()); // Clean up and return
    }
		break;
  case "A": // ' Add new record
		$worth->SendEmail = TRUE; // Send email on add success
    if (AddRow()) { // Add successful
      $_SESSION[EW_SESSION_MESSAGE] = "Add New Record Successful"; // Set up success message
      Page_Terminate($worth->KeyUrl($worth->getReturnUrl())); // Clean up and return
    } else {
...
$worth->RowType = EW_ROWTYPE_ADD;  // Render add type
RenderRow();
?>
...
<p><span class="EziScript">Add to TABLE: Website info<br><br><a href="<?php echo $worth->getReturnUrl() ?>">Go Back</a></span></p>
<?php
...
?>
<form name="fworthadd" id="fworthadd" action="worthadd.php" method="post" onSubmit="return ew_ValidateForm(this);">
<p>
...
function AddRow() {
	...
	$bCheckKey = TRUE;
	$sFilter = $worth->SqlKeyFilter();
	if (trim(strval($worth->id->CurrentValue)) == "") {
		$bCheckKey = FALSE;
	} else {
		$sFilter = str_replace("@id@", ew_AdjustSql($worth->id->CurrentValue), $sFilter); // Replace key value
	}
	...
	$bInsertRow = $worth->Row_Inserting($rsnew);
	if ($bInsertRow) {
		$conn->raiseErrorFn = 'ew_ErrorFn';
		$AddRow = $conn->Execute($worth->InsertSQL($rsnew));
		$conn->raiseErrorFn = '';
	} else {
	...
	if ($AddRow) {
		$worth->id->setDbValue($conn->Insert_ID());
		$rsnew['id'] =& $worth->id->DbValue;

		// Call Row Inserted event
		$worth->Row_Inserted($rsnew);
	}
	return $AddRow;
}
?>
