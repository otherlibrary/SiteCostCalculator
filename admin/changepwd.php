...
$Security->LoadCurrentUserLevel("admin");
?>
<?php
...
if (@$_POST["submit"] <> "") {
	...
	if ($sNPwd == $sCPwd) {
		$sFilter = "(`AdMiN` = '" . ew_AdjustSql($sUsername) . "')";
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				if ((EW_MD5_PASSWORD && md5($sOPwd) == $rs->fields('PaSs')) ||
					(!EW_MD5_PASSWORD && $sOPwd == $rs->fields('PaSs'))) {
					$rsnew = array('PaSs' => $sNPwd); // Change Password
					$sEmail = $rs->fields('email');
					$rs->Close();
					$conn->raiseErrorFn = 'ew_ErrorFn';
					$bValidPwd = $conn->Execute($admin->UpdateSQL($rsnew));
					$conn->raiseErrorFn = '';
					if ($bValidPwd)
						$bPwdUpdated = TRUE;
				}
...
<p><span class="EziScript">Change Password Page</span></p>
...
<table border="0" cellspacing="0" cellpadding="4">
	<tr>
		<td><span class="EziScript">Old Password</span></td>
		<td><span class="EziScript"><input type="password" name="opwd" id="opwd" size="20"></span></td>
	</tr>
	<tr>
		<td><span class="EziScript">New Password</span></td>
		<td><span class="EziScript"><input type="password" name="npwd" id="npwd" size="20"></span></td>
	</tr>
	<tr>
		<td><span class="EziScript">Confirm Password</span></td>
		<td><span class="EziScript"><input type="password" name="cpwd" id="cpwd" size="20"></span></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
		<td><span class="EziScript"><input type="submit" name="submit" id="submit" value="Change Password"></span></td>
	</tr>
</table>
</form>
<br>
