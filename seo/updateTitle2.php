...
if ($contentTypeReal && $contentTypeReal != "utf-8") {
	$correct = iconv($contentTypeReal, "UTF-8", $title);
} else {
	if ($contentTypeReal && $contentTypeReal == "utf-8") {
		$correct = $title;
	} else {
		//$correct = iconv("windows-1251", "UTF-8", $title);
		$correct = iconv(mb_detect_encoding($title), "UTF-8", $title);
		if (strlen($correct) < 2) {
			$correct = $title;
		}
	}
...
include ("config.php");
$sql = "UPDATE worth SET title = '".mysql_real_escape_string($correct)."' WHERE domain = '".$domain."' LIMIT 1";
$rs_result = mysql_query ($sql);
mysql_close($con);
echo $correct;
echo $contentTypeReal;
?>